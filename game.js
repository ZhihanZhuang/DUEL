...
const DEFAULT_BINDS = {
    // 增加了一个 extra 键位，用于触发 Gensan 的剑影（和未来可能的额外技能）
    ...
    update(dt) {
        ...
        if (!this.damageDealt) {
            ...
            if (hitGround) {
                ...
                // 修复：爆炸造成 9 WRD 伤害 (90点) 和 5 秒控制 (5000ms)
...
class Fighter extends Entity {
    ...
    draw(ctx) {
        ...
        if (this.heroName === 'Duke' && this.isMounted) {
            ...
            ctx.fillStyle = "#00FFFF"; ctx.fillRect(-hw, 25, this.w, 4); // Glowing belt
            if (this.isOverloaded && Math.floor(Date.now()/200)%2===0) {
                ctx.fillStyle = "rgba(255, 0, 0, 0.3)"; ctx.fillRect(-hw, 0, this.w, h); // Overload flash
            }
        } else if (this.heroName === 'Gensan') {
            ...
            if (this.gensanSwitchCD <= 0 && this.gensanShadows.length > 0) {
                ctx.fillStyle = "rgba(255,255,255,0.8)"; ctx.fillRect(-hw, -10, this.w, 4); // ready indicator
            }
        } else if (this.heroName === 'Lique') {
            ctx.fillStyle = "#333"; ctx.fillRect(-hw - 2, -2, this.w + 4, h + 4);
            ctx.fillStyle = this.color; ctx.fillRect(-hw, 0, this.w, h);
        ...
        if (this.heroName === 'Hason') {
            ...
            if (enemy) {
                ...
                else if (this.attackState === 'active') { thrust = -30 + (140 * Math.sin(phaseProg * Math.PI)); angle = baseAimAngle; } // longer visual thrust
                ...
                ctx.fillStyle = "#8B4513"; ctx.fillRect(-3, -50 - thrust, 6, 110 + (this.attackState === 'active' ? 40 : 0)); // Spear elongates
        ...
        if (this.aiTimer <= 0) {
            ...
            if (p2.heroName === 'Noae') {
                ...
                if (mineCount < 3 && dist > 150 && Math.random() < 0.05) {
                    scores.switch += 150; // randomly plant mines
                }
                if (dist < 300 && p2.superCooldown <= 0) {
                    scores.superSkill += 200; // use minecart
                }
            }
            ...
            if (p2.heroName === 'Volt') {
                ...
                if (p2.isOverloaded) {
                    ...
                    if (p2.energy > 30) {
                        ...
                        scores.moveBackward += 150; // Try to land safely
                    }
                }
            }
