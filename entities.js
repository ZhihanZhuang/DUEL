...
class FengQigong extends Entity {
    constructor(owner, angle) { super(owner.x + owner.w/2 + owner.facing*18, owner.y + 24, 18, 18); this.owner=owner; this.type='feng_qigong'; this.vx=Math.cos(angle)*16; this.vy=Math.sin(angle)*16; this.life=1500; this.maxLife=1500; this.hitTargets=new Set(); }
    update(dt) { this.x+=this.vx; this.y+=this.vy; this.life-=dt; for (const t of getHostileTargets(this.owner,this)) { if(!t||t.dead||this.hitTargets.has(t)||!checkAABB(this,t)) continue; this.hitTargets.add(t); t.takeDamage(20,this.owner,false,true); t.buffs=t.buffs||{}; t.buffs.slow=Math.max(t.buffs.slow||0,180); for(let i=0;i<8;i++)game.particles.push(new Particle(t.x+t.w/2,t.y+t.h/2,'#dffbff',(Math.random()-.5)*8,(Math.random()-.5)*8,300,3)); this.dead=true; break; } if(this.life<=0||this.x<-40||this.x>CANVAS_W+40||this.y<-40||this.y>CANVAS_H+40)this.dead=true; }
    draw(ctx) { const a=Math.max(0,this.life/this.maxLife); ctx.save();ctx.globalAlpha=a;ctx.shadowBlur=16;ctx.shadowColor='#dffbff';ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(this.x+9,this.y+9,9,0,Math.PI*2);ctx.fill();ctx.strokeStyle='rgba(180,245,255,.65)';ctx.lineWidth=3;ctx.beginPath();ctx.arc(this.x+9,this.y+9,14+Math.sin(Date.now()*.02)*3,0,Math.PI*2);ctx.stroke();ctx.restore(); }
}

class FengWindWave extends Entity {
    constructor(owner, angle=(owner.facing>0?0:Math.PI), ultimate=true) { const direction=Math.cos(angle)>=0?1:-1; super(owner.x + (direction>0?owner.w:-92), owner.y + 4, 92, 78); this.owner=owner; this.type='feng_wind_wave'; this.ultimate=ultimate; this.vx=Math.cos(angle)*12; this.vy=Math.sin(angle)*12; this.damage=20; this.life=2600; this.maxLife=this.life; this.distance=0; this.maxDistance=1650; this.hitTargets=new Set(); this.bounces=0; this.phase=0; this.bounceFlash=0; }
    update(dt) { this.phase+=dt; this.bounceFlash=Math.max(0,this.bounceFlash-dt); const prevX=this.x,prevY=this.y; this.x+=this.vx;this.y+=this.vy;this.distance+=Math.hypot(this.x-prevX,this.y-prevY); this.life-=dt;
        let bounced=false;
        if(this.x<=0||this.x+this.w>=CANVAS_W){this.x=Math.max(0,Math.min(CANVAS_W-this.w,this.x));this.vx*=-1;bounced=true;}
        if(this.y<=0){this.y=0;this.vy=Math.abs(this.vy||1);bounced=true;}
        if(this.y+this.h>=GROUND_Y){this.y=GROUND_Y-this.h;this.vy=-Math.abs(this.vy||1);bounced=true;}
        for(const p of (PLATFORMS||[])){const overlap=this.x+this.w>p.x&&this.x<p.x+p.w,ph=p.h||10;if(!overlap)continue;if(prevY+this.h<=p.y&&this.y+this.h>=p.y&&this.vy>0){this.y=p.y-this.h;this.vy=-Math.abs(this.vy);bounced=true;}else if(prevY>=p.y+ph&&this.y<=p.y+ph&&this.vy<0){this.y=p.y+ph;this.vy=Math.abs(this.vy);bounced=true;}}
        if(bounced){this.bounces++;this.bounceFlash=120;this.vx*=1.035;this.vy*=1.035;for(let i=0;i<14;i++)game.particles.push(new Particle(this.x+this.w/2,this.y+this.h/2,'#dffbff',(Math.random()-.5)*11,(Math.random()-.5)*11,300,3));}
        for(const t of getHostileTargets(this.owner,this)){if(!t||t.dead||this.hitTargets.has(t)||!checkAABB(this,t))continue;this.hitTargets.add(t);t.takeDamage(this.damage,this.owner,false,true);t.buffs=t.buffs||{};t.buffs.slow=Math.max(t.buffs.slow||0,750);const speed=Math.max(1,Math.hypot(this.vx,this.vy)),nx=this.vx/speed,ny=this.vy/speed;t.vx=nx*30;t.vy=ny*22-5;for(let i=0;i<20;i++)game.particles.push(new Particle(t.x+t.w/2,t.y+t.h/2,'#eaffff',nx*(6+Math.random()*12)+(Math.random()-.5)*7,ny*(6+Math.random()*12)+(Math.random()-.5)*7,400,4));}
        if(this.life<=0||this.distance>=this.maxDistance)this.dead=true;
    }
    draw(ctx) { const t=this.phase*.014,compression=this.bounceFlash>0?.58:1,rear=-this.w*.46,front=this.w*.5;ctx.save();ctx.globalAlpha=Math.max(.08,this.life/this.maxLife);ctx.translate(this.x+this.w/2,this.y+this.h/2);ctx.rotate(Math.atan2(this.vy,this.vx));ctx.scale(compression,1);ctx.shadowBlur=17;ctx.shadowColor='#dffbff';ctx.fillStyle='rgba(218,250,255,.18)';ctx.beginPath();ctx.moveTo(rear,0);ctx.quadraticCurveTo(0,-this.h*.62,front,-this.h*.46);ctx.lineTo(front,this.h*.46);ctx.quadraticCurveTo(0,this.h*.62,rear,0);ctx.fill();for(let i=0;i<6;i++){const side=i%2?-1:1,spread=(.16+i*.065)*this.h,flutter=Math.sin(t+i*1.7)*5;ctx.strokeStyle=i%2?'rgba(175,237,248,.72)':'rgba(255,255,255,.94)';ctx.lineWidth=2.5+(i%3);ctx.beginPath();ctx.moveTo(rear-i*2,flutter*.22);ctx.quadraticCurveTo(-8,side*(spread*.72)+flutter,front+i*3,side*spread);ctx.stroke();}ctx.strokeStyle='rgba(238,255,255,.75)';ctx.lineWidth=3;ctx.beginPath();ctx.arc(rear+4,0,15+Math.sin(t)*4,-1.15,1.15);ctx.stroke();ctx.restore(); }
}
...
class OcelRitualZone extends Entity {
    constructor(owner) {
    ...
    draw(ctx) {
        const fade=Math.min(1,this.life/450),t=this.phase*.0025,cx=this.x+this.w/2,cy=this.y+12;
        ctx.save();ctx.globalAlpha=fade;ctx.translate(cx,cy);ctx.scale(1,.28);ctx.shadowBlur=18;ctx.shadowColor='#32ddca';
        ctx.strokeStyle='rgba(55,224,204,.86)';ctx.lineWidth=8;ctx.beginPath();ctx.arc(0,0,132,0,Math.PI*2);ctx.stroke();
        ctx.rotate(t);ctx.strokeStyle='#e8bd43';ctx.lineWidth=5;for(let i=0;i<12;i++){ctx.rotate(Math.PI/6);ctx.beginPath();ctx.moveTo(70,0);ctx.lineTo(122,0);ctx.stroke();}
        ctx.rotate(-t*2.1);ctx.strokeStyle='#55f0d9';ctx.lineWidth=7;ctx.beginPath();for(let i=0;i<=12;i++){const a=i*Math.PI/6,r=i%2?43:66;const x=Math.cos(a)*r,y=Math.sin(a)*r;i?ctx.lineTo(x,y):ctx.moveTo(x,y);}ctx.closePath();ctx.stroke();ctx.restore();
        ctx.save();ctx.globalAlpha=.18*fade;ctx.fillStyle='#21bb83';ctx.fillRect(this.x,this.y-90,this.w,105);ctx.restore();
    }
}
...
class ElectromagneticMatrix extends Entity {
    constructor(owner,angle,overcharged=false){
        const w=overcharged?142:112,h=overcharged?82:64,direction=Math.cos(angle)>=0?1:-1;
        super(owner.x+(direction>0?owner.w:-w),owner.y+owner.h*.35-h/2,w,h);
        this.owner=owner;this.type='electromagnetic_matrix';this.angle=angle;this.overcharged=overcharged;this.damage=overcharged?90:60;
        this.speed=overcharged?7.2:6.1;this.vx=Math.cos(angle)*this.speed;this.vy=Math.sin(angle)*this.speed;this.life=3600;this.maxLife=this.life;this.distance=0;this.hitTargets=new Set();this.stackGranted=false;
    }
    update(dt){
        const oldX=this.x,oldY=this.y;this.x+=this.vx;this.y+=this.vy;this.distance+=Math.hypot(this.x-oldX,this.y-oldY);this.life-=dt;
        for(const target of getHostileTargets(this.owner,this)){
            if(!target||target.dead||target.untargetable||this.hitTargets.has(target)||!checkAABB(this,target))continue;
            this.hitTargets.add(target);target.takeDamage(this.damage,this.owner,false,true);
            if(this.overcharged){target.buffs=target.buffs||{};target.buffs.dizzy=Math.max(target.buffs.dizzy||0,800);target.vx=Math.sign(this.vx)*8;}
            else if(!this.stackGranted){this.stackGranted=true;this.owner.magnetarOverload=Math.min(3,(this.owner.magnetarOverload||0)+1);}
            for(let i=0;i<18;i++)game.particles.push(new Particle(target.x+target.w/2,target.y+target.h/2,i%3?'#72e8ff':'#ffffff',(Math.random()-.5)*12,(Math.random()-.5)*12,380,3));
        }
        if(this.life<=0||this.distance>1550||this.x<-180||this.x>CANVAS_W+180||this.y<-160||this.y>CANVAS_H+160)this.dead=true;
    }
    draw(ctx){
        const pulse=.75+Math.sin(Date.now()*.022)*.2,fade=Math.min(1,this.life/280);ctx.save();ctx.globalAlpha=fade;ctx.translate(this.x+this.w/2,this.y+this.h/2);ctx.rotate(this.angle);ctx.shadowBlur=this.overcharged?25:17;ctx.shadowColor=this.overcharged?'#e8fbff':'#38cfff';
        ctx.fillStyle=this.overcharged?'rgba(191,245,255,.22)':'rgba(40,157,214,.16)';ctx.fillRect(-this.w/2,-this.h/2,this.w,this.h);
        ctx.strokeStyle=this.overcharged?'#f4ffff':'#72e7ff';ctx.lineWidth=this.overcharged?4:3;ctx.strokeRect(-this.w/2,-this.h/2,this.w,this.h);
        const columns=this.overcharged?7:6,rows=this.overcharged?5:4;ctx.lineWidth=1.5;
        for(let i=1;i<columns;i++){ctx.strokeStyle=`rgba(151,239,255,${pulse})`;ctx.beginPath();ctx.moveTo(-this.w/2+i*this.w/columns,-this.h/2);ctx.lineTo(-this.w/2+i*this.w/columns,this.h/2);ctx.stroke();}
        for(let i=1;i<rows;i++){ctx.strokeStyle=`rgba(225,253,255,${pulse*.88})`;ctx.beginPath();ctx.moveTo(-this.w/2,-this.h/2+i*this.h/rows);ctx.lineTo(this.w/2,-this.h/2+i*this.h/rows);ctx.stroke();}
        if(this.overcharged){ctx.strokeStyle='#fff59c';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-this.w*.35,0);ctx.lineTo(-this.w*.18,-13);ctx.lineTo(0,13);ctx.lineTo(this.w*.18,-13);ctx.lineTo(this.w*.35,0);ctx.stroke();}
        ctx.restore();
    }
}

class MagneticRepulsion extends Entity {
    constructor(owner){super(owner.x+owner.w/2-190,owner.y+owner.h/2-190,380,380);this.owner=owner;this.type='magnetic_repulsion';this.life=320;this.maxLife=320;this.triggered=false;this.untargetable=true;}
    update(dt){
        this.x=this.owner.x+this.owner.w/2-190;this.y=this.owner.y+this.owner.h/2-190;this.life-=dt;
        if(!this.triggered){this.triggered=true;const cx=this.x+190,cy=this.y+190;
            for(const target of getHostileTargets(this.owner,this)){if(!target||target.dead)continue;const tx=target.x+target.w/2,ty=target.y+target.h/2,dx=tx-cx,dy=ty-cy,distance=Math.hypot(dx,dy);if(distance>190)continue;target.takeDamage(20,this.owner,false,true);const nx=dx/(distance||1),ny=dy/(distance||1);target.vx=nx*25;target.vy=ny*15-7;}
            for(const projectile of game.projectiles){if(!projectile||projectile.dead||projectile.owner===this.owner||Math.max(projectile.w||0,projectile.h||0)>44)continue;const px=projectile.x+(projectile.w||0)/2,py=projectile.y+(projectile.h||0)/2;if(Math.hypot(px-cx,py-cy)<=205)projectile.dead=true;}
            for(let i=0;i<42;i++){const a=i*Math.PI/21;game.particles.push(new Particle(cx+Math.cos(a)*35,cy+Math.sin(a)*35,i%3?'#7cecff':'#ffffff',Math.cos(a)*(7+Math.random()*10),Math.sin(a)*(7+Math.random()*10),420,4));}
        }
        if(this.life<=0)this.dead=true;
    }
    draw(ctx){const progress=1-this.life/this.maxLife,radius=40+progress*165;ctx.save();ctx.globalAlpha=Math.max(0,1-progress);ctx.translate(this.x+190,this.y+190);ctx.strokeStyle='#a7f4ff';ctx.shadowBlur=25;ctx.shadowColor='#4bdcff';ctx.lineWidth=9-progress*5;ctx.beginPath();ctx.arc(0,0,radius,0,Math.PI*2);ctx.stroke();ctx.strokeStyle='rgba(255,255,255,.72)';ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,radius*.74,0,Math.PI*2);ctx.stroke();ctx.restore();}
}

class MatrixBombardment extends Entity {
    constructor(owner,target){
        const center=target&&!target.dead?target.x+target.w/2:owner.x+owner.w/2+owner.facing*300;
        super(Math.max(20,Math.min(CANVAS_W-620,center-310)),0,620,GROUND_Y);this.owner=owner;this.type='matrix_bombardment';this.elapsed=0;this.warning=3000;this.strikeInterval=430;this.strikeXs=[55,155,255,365,465,565].map(offset=>this.x+offset);this.struck=[false,false,false,false,false,false];this.fieldLives=[0,0,0,0,0,0];this.life=6580;this.untargetable=true;
    }
    update(dt){
        if(!this.owner||this.owner.dead){this.dead=true;return;}this.elapsed+=dt;this.life-=dt;
        for(let i=0;i<this.fieldLives.length;i++)this.fieldLives[i]=Math.max(0,this.fieldLives[i]-dt);
        for(let i=0;i<6;i++){const impactTime=this.warning+i*this.strikeInterval;if(this.struck[i]||this.elapsed<impactTime)continue;this.struck[i]=true;this.fieldLives[i]=950;const strikeX=this.strikeXs[i];
            for(const target of getHostileTargets(this.owner,this)){if(!target||target.dead)continue;const tx=target.x+target.w/2;if(Math.abs(tx-strikeX)>72)continue;target.takeDamage(100,this.owner,false,true);target.buffs=target.buffs||{};target.buffs.slow=Math.max(target.buffs.slow||0,1500);target.vy=-7;}
            for(let p=0;p<28;p++)game.particles.push(new Particle(strikeX,GROUND_Y-15,p%3?'#72eaff':'#ffffff',(Math.random()-.5)*15,-4-Math.random()*15,520,4));
        }
        for(let i=0;i<6;i++){if(this.fieldLives[i]<=0)continue;for(const target of getHostileTargets(this.owner,this)){if(!target||target.dead)continue;if(Math.abs(target.x+target.w/2-this.strikeXs[i])<=82&&target.y+target.h>=GROUND_Y-45){target.buffs=target.buffs||{};target.buffs.slow=Math.max(target.buffs.slow||0,180);}}}
        if(this.life<=0)this.dead=true;
    }
    draw(ctx){
        ctx.save();const warningActive=this.elapsed<this.warning,pulse=.42+Math.sin(Date.now()*.018)*.18;
        if(warningActive){ctx.fillStyle=`rgba(42,142,201,${.08+pulse*.08})`;ctx.fillRect(this.x,0,this.w,GROUND_Y);ctx.strokeStyle=`rgba(129,236,255,${.62+pulse*.3})`;ctx.lineWidth=4;ctx.setLineDash([14,9]);ctx.strokeRect(this.x+2,3,this.w-4,GROUND_Y-7);ctx.setLineDash([]);
            ctx.strokeStyle='rgba(113,226,255,.38)';ctx.lineWidth=2;for(let gx=this.x;gx<=this.x+this.w;gx+=52){ctx.beginPath();ctx.moveTo(gx,0);ctx.lineTo(gx,GROUND_Y);ctx.stroke();}for(let gy=35;gy<GROUND_Y;gy+=54){ctx.beginPath();ctx.moveTo(this.x,gy);ctx.lineTo(this.x+this.w,gy);ctx.stroke();}
            for(const strikeX of this.strikeXs){ctx.fillStyle=`rgba(175,246,255,${.13+pulse*.12})`;ctx.fillRect(strikeX-68,GROUND_Y-24,136,24);ctx.strokeStyle='#a7f5ff';ctx.strokeRect(strikeX-68,GROUND_Y-24,136,24);}
        }else{
            for(let i=0;i<6;i++){const impactTime=this.warning+i*this.strikeInterval,fall=Math.max(0,Math.min(1,(this.elapsed-impactTime+300)/300));if(!this.struck[i]&&fall>0){const top=-140+(GROUND_Y+120)*fall;ctx.fillStyle='rgba(105,226,255,.28)';ctx.fillRect(this.strikeXs[i]-63,top-130,126,130);ctx.strokeStyle='#d7fbff';ctx.lineWidth=4;ctx.strokeRect(this.strikeXs[i]-63,top-130,126,130);for(let line=1;line<5;line++){ctx.beginPath();ctx.moveTo(this.strikeXs[i]-63+line*25,top-130);ctx.lineTo(this.strikeXs[i]-63+line*25,top);ctx.stroke();}}
                if(this.fieldLives[i]>0){const alpha=this.fieldLives[i]/950;ctx.fillStyle=`rgba(51,190,232,${alpha*.22})`;ctx.fillRect(this.strikeXs[i]-82,GROUND_Y-38,164,38);ctx.strokeStyle=`rgba(169,246,255,${alpha})`;ctx.lineWidth=3;ctx.strokeRect(this.strikeXs[i]-82,GROUND_Y-38,164,38);}}
        }ctx.restore();
    }
}

class BlackSpike extends Entity {
    constructor(owner,x,y,angle,ignoredTarget=null){super(x-7,y-3,14,6);this.owner=owner;this.type='black_spike';this.angle=angle;this.vx=Math.cos(angle)*19;this.vy=Math.sin(angle)*19;this.damage=5*(owner.getNerathPower?.()||1);this.life=430;this.ignoredTarget=ignoredTarget;this.untargetable=true;}
    update(dt){this.x+=this.vx;this.y+=this.vy;this.life-=dt;for(const target of getHostileTargets(this.owner,this)){if(target===this.ignoredTarget||!checkAABB(this,target))continue;target.takeDamage(this.damage,this.owner,false,true);this.dead=true;break;}if(this.life<=0||this.x<-30||this.x>CANVAS_W+30||this.y<-30||this.y>CANVAS_H+30)this.dead=true;}
    draw(ctx){ctx.save();ctx.translate(this.x+7,this.y+3);ctx.rotate(this.angle);ctx.fillStyle='#10080e';ctx.strokeStyle='#9e2948';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(8,0);ctx.lineTo(-4,-4);ctx.lineTo(-8,0);ctx.lineTo(-4,4);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();}
}

class BlackShard extends Entity {
    constructor(owner,angle){const x=owner.x+owner.w/2+Math.cos(angle)*28,y=owner.y+owner.h*.38+Math.sin(angle)*28;super(x-17,y-13,34,26);this.owner=owner;this.type='black_shard';this.angle=angle;this.rotation=0;this.vx=Math.cos(angle)*27;this.vy=Math.sin(angle)*27;this.damage=15*(owner.getNerathPower?.()||1);this.life=720;this.distance=0;this.untargetable=true;}
    shatter(target){if(this.dead)return;this.dead=true;const cx=this.x+this.w/2,cy=this.y+this.h/2;[-.68,-.24,.24,.68].forEach((offset,index)=>game.projectiles.push(new BlackSpike(this.owner,cx,cy,this.angle+offset,index===1?null:target)));for(let i=0;i<16;i++)game.particles.push(new Particle(cx,cy,i%3?'#160a12':'#a52342',(Math.random()-.5)*13,(Math.random()-.5)*13,300,3));}
    update(dt){this.x+=this.vx;this.y+=this.vy;this.rotation+=.34;this.distance+=Math.hypot(this.vx,this.vy);this.life-=dt;for(const target of getHostileTargets(this.owner,this)){if(!target||target.dead||!checkAABB(this,target))continue;target.takeDamage(this.damage,this.owner,false,true);this.shatter(target);return;}if(this.life<=0||this.distance>680||this.x<-40||this.x>CANVAS_W+40||this.y<-40||this.y>CANVAS_H+40)this.dead=true;}
    draw(ctx){ctx.save();ctx.translate(this.x+this.w/2,this.y+this.h/2);ctx.rotate(this.rotation);ctx.shadowBlur=18;ctx.shadowColor='#c52549';ctx.fillStyle='#09060a';ctx.strokeStyle='#9e2948';ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(0,-16);ctx.lineTo(18,0);ctx.lineTo(0,16);ctx.lineTo(-18,0);ctx.closePath();ctx.fill();ctx.stroke();ctx.strokeStyle='rgba(244,75,110,.78)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,-12);ctx.lineTo(0,12);ctx.moveTo(-13,0);ctx.lineTo(13,0);ctx.stroke();ctx.fillStyle='rgba(255,174,192,.9)';ctx.beginPath();ctx.arc(0,0,2.5,0,Math.PI*2);ctx.fill();ctx.restore();}
}

class HellHand extends Entity {
    constructor(owner,target,side,castId){const cx=target.x+target.w/2;super(Math.max(0,Math.min(CANVAS_W-48,cx+side*300-24)),target.y+target.h/2-25,48,50);this.owner=owner;this.targetId=target.id;this.side=side;this.castId=castId;this.type='hell_hand';this.hp=30;this.maxHp=30;this.life=4200;this.phase='tracking';this.grabbed=false;this.holdTimer=0;this.vx=0;this.vy=0;this.buffs={};}
...
class HellTearEffect extends Entity {
    constructor(x,y){super(x-95,y-65,190,130);this.type='hell_tear_effect';this.life=430;this.maxLife=430;this.untargetable=true;}
    update(dt){this.life-=dt;if(this.life<=0)this.dead=true;}
    draw(ctx){const p=1-this.life/this.maxLife;ctx.save();ctx.translate(this.x+95,this.y+65);ctx.globalAlpha=Math.max(0,1-p);ctx.strokeStyle='#ef3159';ctx.shadowBlur=18;ctx.shadowColor='#c01338';ctx.lineWidth=8;ctx.lineCap='round';for(const side of [-1,1])for(let slash=0;slash<3;slash++){const outward=(20+p*62)*side,y=(slash-1)*19;ctx.beginPath();ctx.moveTo(side*10,y-18);ctx.lineTo(outward,y+18);ctx.stroke();}ctx.restore();}
}
...
class NerathResurrection extends Entity {
    constructor(owner){super(owner.x-90,owner.y-35,owner.w+180,owner.h+80);this.owner=owner;this.type='nerath_resurrection';this.life=1500;this.maxLife=1500;this.untargetable=true;}
    update(dt){this.x=this.owner.x-90;this.y=this.owner.y-35;this.life-=dt;if(this.life<=0)this.dead=true;}
    draw(ctx){const p=1-this.life/this.maxLife,cx=this.x+this.w/2,cy=this.y+this.h*.62;ctx.save();ctx.globalAlpha=Math.sin(Math.min(1,p)*Math.PI);ctx.translate(cx,cy);ctx.fillStyle='rgba(6,1,6,.9)';ctx.strokeStyle='#bd2749';ctx.shadowBlur=25;ctx.shadowColor='#9a1635';ctx.lineWidth=7;ctx.beginPath();ctx.ellipse(0,0,75,105,0,0,Math.PI*2);ctx.fill();ctx.stroke();for(let i=0;i<12;i++){const a=i*Math.PI/6+p*2;ctx.fillStyle=i%2?'#220814':'#a92343';ctx.beginPath();ctx.moveTo(Math.cos(a)*55,Math.sin(a)*82);ctx.lineTo(Math.cos(a+.15)*85,Math.sin(a+.15)*118);ctx.lineTo(Math.cos(a-.15)*68,Math.sin(a-.15)*94);ctx.fill();}ctx.restore();}
}
