...
class BlackShard extends Entity {
    constructor(owner,angle){const x=owner.x+owner.w/2+Math.cos(angle)*24,y=owner.y+owner.h*.38+Math.sin(angle)*24;super(x-11,y-8,22,16);this.owner=owner;this.type='black_shard';this.angle=angle;this.rotation=0;this.vx=Math.cos(angle)*27;this.vy=Math.sin(angle)*27;this.damage=12*(owner.getNerathPower?.()||1);this.life=720;this.distance=0;this.untargetable=true;}
    ...
    draw(ctx){ctx.save();ctx.translate(this.x+this.w/2,this.y+this.h/2);ctx.rotate(this.rotation);ctx.shadowBlur=12;ctx.shadowColor='#a51f3d';ctx.fillStyle='#09060a';ctx.strokeStyle='#7d2038';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,-11);ctx.lineTo(12,0);ctx.lineTo(0,11);ctx.lineTo(-12,0);ctx.closePath();ctx.fill();ctx.stroke();ctx.strokeStyle='rgba(229,60,93,.65)';ctx.beginPath();ctx.moveTo(0,-8);ctx.lineTo(0,8);ctx.moveTo(-8,0);ctx.lineTo(8,0);ctx.stroke();ctx.restore();}
}
