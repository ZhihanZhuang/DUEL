...
// --- 3. 动态劫持单机游戏循环 (智能轮询等待 game.js 加载) ---
function applyNetworkPatch() {
    // 检查 game.js 是否已经成功把 Game 类挂载到了 window 上
    if (window.Game && window.Game.prototype) {
        ...
        setTimeout(applyNetworkPatch, 500); // 轮询等待
    }
}
// 启动补丁注入程序
applyNetworkPatch();
    ...
    window.currentRoomUnsub = onSnapshot(roomRef, (snap) => {
        ...
        if (window.isHost) {
            if (data.clientId) {
                ...
                btnStart.style.display = 'none'; statusText.innerText = "等待挑战者加入...";
            }
        } else {
            if (data.status === 'waiting') statusText.innerText = "等待房主开始游戏...";
            else if (data.status === 'starting') {
                statusText.innerText = "正在建立网络直连...";
